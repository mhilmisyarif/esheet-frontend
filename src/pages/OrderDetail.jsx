...
export default function OrderDetail() {
  ...
  useEffect(() => {
    ...
    // Fetch the single order from the real API
    apiClient
      .get(`/orders/${orderId}`)
      .then((response) => {
        if (mounted) {
          setOrder(response.data); // Set the order object from response
        }
      })
      .catch((err) => {
  ...
  return (
    <div>
      ...
      <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* Make sure your /orders/:orderId endpoint also returns the 'samples' array */}
        {order.samples &&
          order.samples.map((s) => (
            <div key={s.id} className="bg-white rounded shadow p-4">
              <div className="flex justify-between">
                ...
                <div className="flex flex-col gap-2">
                  {/* This link correctly goes to the ReportEditor, which uses the real API */}
                  <Link
                    to={`/reports/${s.id}`}
