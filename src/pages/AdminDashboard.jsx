...
// Reusing constants and helper components from TechnicianDashboard
const MONTHS = [
  "January",
  "February",
  "March",
...
// Reusing Tabs concept from EngineerDashboard
const TABS = [
  { key: "DATASHEETS", label: "All Datasheets" },
...
// Reusing FilterDropdown exactly from TechnicianDashboard[cite: 3]
...
export default function AdminDashboard() {
  ...
  // Fetch samples exactly like TechnicianDashboard[cite: 3]
  useEffect(() => {
    let mounted = true;
    apiClient
      .get("/samples")
      .then((res) => {
        ...
        const mapped = list.map((s) => ({
          ...
          sampleName: s.name || "—",
          brandModel: [s.brand, s.model].filter(Boolean).join(" ") || "—",
      ...
      .catch((err) => {
        ...
        toast.error("Failed to load datasheets.");
      })
      .finally(() => {
  ...
  // Downloading datasheet remains exactly the same[cite: 3]
  async function downloadDatasheet(row) {
    if (!row.reportId) {
      toast.error("This datasheet does not have a report yet.");
      return;
    }
    try {
      ...
      toast.error("Failed to download datasheet.");
    }
  }
    ...
    <div>
      ...
      <div className="flex flex-col gap-4 nav:flex-row nav:items-center nav:justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          {/* Reusing Tabs style from EngineerDashboard[cite: 4] */}
          <div className="inline-flex p-1 bg-navy-50 rounded-xl self-start mr-2">
            {TABS.map((t) => (
              <button
                key={t.key}
          ...
          <button
            ...
            className="inline-flex items-center justify-center gap-2 h-11 px-[18px] rounded-xl text-sm font-semibold bg-navy-800 text-white hover:bg-navy-700 transition-colors whitespace-nowrap"
          >
            <FiUserPlus size={18} /> Register User
          </button>
        </div>
      </div>
      ...
      {tab === "DATASHEETS" && (
        <>
          {/* Filters - Reused from TechnicianDashboard[cite: 3] */}
          <div className="flex flex-col gap-2 nav:flex-row nav:justify-end mb-4">
          ...
          <section className="bg-paper border border-line rounded-2xl shadow-card overflow-hidden">
            ...
            {/* Desktop table - Extracted directly from TechnicianDashboard[cite: 3] */}
                ...
                <tbody>
                  {loading ? (
                    ...
                    pageRows.map((r, i) => (
                      ...
                      >
                        <td
                          className={`${CELL} text-sm text-ink-400 tabular-nums`}
                        >
                        ...
                        <td
                          className={`${CELL} text-sm font-medium text-navy-800`}
                        >
                          {r.client}
                        </td>
                        ...
                        <td className={`${CELL} text-right`}>
                          <button
                            ...
                            className="inline-flex w-8 h-8 rounded-lg items-center justify-center text-ink-400 hover:bg-navy-100 hover:text-navy-800 transition-colors"
                          >
            ...
            <div className="flex flex-col gap-3 nav:flex-row nav:items-center nav:justify-between px-4 nav:px-6 py-4 border-t border-line-soft text-[13px] text-ink-400">
              <span>
                ...
                {(safePage - 1) * PER_PAGE + pageRows.length} of{" "}
                {filtered.length}
              </span>
              <div className="flex items-center justify-center flex-wrap gap-1">
                <button
                  ...
                  className="inline-flex items-center gap-1 h-9 px-2.5 rounded-lg text-[13px] font-medium text-navy-800 hover:bg-navy-50 disabled:text-ink-300 disabled:cursor-not-allowed"
                >
                ...
                {Array.from({ length: pages }).map((_, i) => (
                  <button
                    key={i}
                    onClick={() => setPage(i + 1)}
                    className={`min-w-9 h-9 px-2.5 rounded-lg text-[13px] font-medium transition-colors ${safePage === i + 1 ? "bg-navy-800 text-white" : "text-navy-800 hover:bg-navy-50"}`}
                  >
                    {i + 1}
                  </button>
                ))}
      ...
      {tab === "STANDARDS" && (
        <section className="bg-paper border border-line rounded-2xl shadow-card overflow-hidden p-12 flex flex-col items-center justify-center text-center">
          <div className="w-16 h-16 bg-navy-50 text-navy-600 rounded-full flex items-center justify-center mb-4">
            <FiSettings size={28} />
          </div>
          ...
          <p className="text-sm text-ink-400 max-w-md">
            This section is ready for your Standards CRUD interface. You can
            fetch and display the available test standards and clauses here.
          </p>
          {/* Add your standards table/management UI here in the future! */}
        </section>
      )}

      {/* Row-action menu (Restricted view - no delete) */}
      {menu && (
        <>
          ...
          >
            ...
            >
              <FiFileText size={15} /> View Datasheet
            </button>
